...
// 引入工程国际化语言资源
import zhLocale from '@/assets/locals/zh-CN.js'
// import enLocale from '@/assets/locals/en-US.js'
...
class I18nManager {
  ...
  constructor() {
    ...
    let prevLanguage = window.localStorage.getItem(STORAGE_KEY)

    if (!prevLanguage) {
      localLanguage = SUPORTED_LIST.includes(browserLanguage) ? browserLanguage : DEFAULT_LANG
    } else {
      localLanguage = SUPORTED_LIST.includes(prevLanguage) ? prevLanguage : DEFAULT_LANG
    }
    ...
    if (prevLanguage !== localLanguage) {
      this.loadLanguageAsync(prevLanguage)
    }
  }
